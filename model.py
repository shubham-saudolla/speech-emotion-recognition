...
tf.app.flags.DEFINE_float('dropout_keep_prob',     1,
                          'the prob of every unit keep in dropout layer')
...
tf.app.flags.DEFINE_string(
    'traindata_path', './IEMOCAP.pkl', 'total dataset includes training set')
tf.app.flags.DEFINE_string(
    'validdata_path', 'inputs/valid.pkl', 'total dataset includes valid set')
...
def train():
    ...
    X = tf.placeholder(tf.float32, shape=[
                       None, FLAGS.image_height, FLAGS.image_width, FLAGS.image_channel])
    ...
    with tf.Session() as sess:
        ...
        for i in range(FLAGS.num_epoch):
            ...
            if i % 5 == 0:
                ...
                y_pred_valid = np.empty(
                    (valid_size, FLAGS.num_classes), dtype=np.float32)
                y_valid = np.empty((vnum, 4), dtype=np.float32)
                index = 0
                cost_valid = 0
                if(valid_size < FLAGS.batch_size):
                    loss, y_pred_valid = sess.run([cross_entropy, Ylogits], feed_dict={
                ...
                if valid_acc_uw > best_valid_uw:
                    ...
                    saver.save(sess, os.path.join(FLAGS.checkpoint,
                                                  FLAGS.model_name), global_step=i+1)
                print(
                    "*****************************************************************")
                print("Epoch: %05d" % (i+1))
