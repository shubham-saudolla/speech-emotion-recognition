            ...
            with tf.Session() as sess:
                ...
                if ckpt and ckpt.model_checkpoint_path:
                    ...
                    if(valid_size < FLAGS.valid_batch_size):
                        validate_feed = {
                            model.inputs: valid_data, model.labels: Valid_label}
                        y_pred_valid, loss = sess.run(
                            [model.logits, cross_entropy], feed_dict=validate_feed)
                    ...
                    for v in range(valid_iter):
                        ...
                        if(v == valid_iter-1):
                            if(v_end < valid_size):
                                v_end = valid_size
                        validate_feed = {
                            model.inputs: valid_data[v_begin:v_end], model.labels: Valid_label[v_begin:v_end]}
                        loss, y_pred_valid[v_begin:v_end, :] = sess.run(
                            [cross_entropy, model.logits], feed_dict=validate_feed)
                        cost_valid = cost_valid + np.sum(loss)
                    ...
                    valid_acc_uw = recall(np.argmax(valid_label, 1), np.argmax(
                        y_valid, 1), average='macro')
                    valid_acc_w = recall(np.argmax(valid_label, 1), np.argmax(
                        y_valid, 1), average='weighted')
                    valid_conf = confusion(
                        np.argmax(valid_label, 1), np.argmax(y_valid, 1))
                    ...
                    for t in range(test_iter):
                        ...
                        test_feed = {
                            model.inputs: test_data[t_begin:t_end], model.labels: Test_label[t_begin:t_end]}
                    ...
                    if valid_acc_uw > best_valid_uw:
                        ...
                        pred_test_uw = y_test
                        test_acc_uw = recall(np.argmax(test_label, 1), np.argmax(
                            y_test, 1), average='macro')
                        test_conf = confusion(
                            np.argmax(test_label, 1), np.argmax(y_test, 1))
                    ...
                    if valid_acc_w > best_valid_w:
                        ...
                        pred_test_w = y_test
                        test_acc_w = recall(np.argmax(test_label, 1), np.argmax(
                            y_test, 1), average='weighted')
                        test_conf = confusion(
                            np.argmax(test_label, 1), np.argmax(y_test, 1))
                    ...
                    if(flag):
                        ...
                        cPickle.dump((best_valid_uw, best_valid_w, pred_test_w, test_acc_w,
                                      confusion_w, pred_test_uw, test_acc_uw, confusion_uw,), f)
                        f.close()
                        flag = False
