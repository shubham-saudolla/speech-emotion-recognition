...
def read_IEMOCAP():
    ...
    train_num = 2928
    filter_num = 40
    # remerber each utterance contain how many segments
                ...
                if(sess[7] == 'i'):
                    ...
                    for filename in files:
                        ...
                        if(emotion in ['hap', 'ang', 'neu', 'sad']):
                            ...
                            mel_spec = ps.logfbank(
                                data, rate, nfilt=filter_num)
                            ...
                            if(speaker in ['Session1', 'Session2', 'Session3', 'Session4']):
                                ...
                                if(time <= 300):
                                    ...
                                    train_data[train_num, :, :, 0] = (
                                        part - mean1)/(std1+eps)
                                    train_data[train_num, :, :, 1] = (
                                        delta11 - mean2)/(std2+eps)
                                    train_data[train_num, :, :, 2] = (
                                        delta21 - mean3)/(std3+eps)
                                ...
                                if(wavname[-4] == 'M'):
                                    ...
                                    if(time <= 300):
                                        ...
                                        test_data[test_num, :, :, 0] = (
                                            part - mean1)/(std1+eps)
                                        test_data[test_num, :, :, 1] = (
                                            delta11 - mean2)/(std2+eps)
                                        test_data[test_num, :, :, 2] = (
                                            delta21 - mean3)/(std3+eps)
                                    ...
                                    if(time <= 300):
                                        ...
                                        delta21 = np.pad(
                                            delta21, ((0, 300 - delta21.shape[0]), (0, 0)), 'constant', constant_values=0)
                                        valid_data[valid_num, :, :, 0] = (
                                            part - mean1)/(std1+eps)
                                        valid_data[valid_num, :, :, 1] = (
                                            delta11 - mean2)/(std2+eps)
                                        valid_data[valid_num, :, :, 2] = (
                                            delta21 - mean3)/(std3+eps)
    ...
    for m in range(1):
        ...
        Train_label = np.empty((train_num, 1), dtype=np.int8)
        Train_data = np.empty(
            (train_num, 300, filter_num, 3), dtype=np.float32)
        ...
        output = './IEMOCAP.pkl'
        f = open(output, 'wb')
        cPickle.dump((Train_data, Train_label, test_data, test_label, valid_data,
                      valid_label, Valid_label, Test_label, pernums_test, pernums_valid), f)
        f.close()
    return
