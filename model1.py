...
def build_model(inputX, is_training, keep_prob):
    ...
    layer1 = tf.nn.max_pool(layer1, ksize=[1, 1, 4, 1], strides=[
                            1, 1, 4, 1], padding='VALID', name='max_pool')
    # print layer1.get_shape()
    layer1 = tf.contrib.layers.dropout(
        layer1, keep_prob=keep_prob, is_training=is_training)
    ...
    layer2 = tf.nn.max_pool(layer2, ksize=[1, 1, 2, 1], strides=[
                            1, 1, 2, 1], padding='VALID', name='max_pool')
    # print layer2.get_shape()
    layer2 = tf.contrib.layers.dropout(
        layer2, keep_prob=keep_prob, is_training=is_training)
    ...
    gru = tf.nn.max_pool(outputs, ksize=[1, timesteps, 1, 1], strides=[
                         1, timesteps, 1, 1], padding='VALID', name='max_pool')
...
def train_op(norm):
    ...
    if norm == -1:
        ...
        grads, _ = tf.clip_by_global_norm(
            tf.gradients(cost, var_trainable_op), grad_clip)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 5 == 0:
                loss, train_acc = sess.run([cost, accuracy], feed_dict={
                                           X: valid_data, Y: valid_label, is_training: False, keep_prob: 1})
                test_acc = sess.run(accuracy, feed_dict={
                                    X: test_data, Y: test_label, is_training: False, keep_prob: 1})
                ...
                saver.save(sess, os.path.join(
                    MODEL_SAVE_PATH, MODEL_NAME), global_step=i)
