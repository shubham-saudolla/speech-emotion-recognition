...
tf.app.flags.DEFINE_string('data_path',  './IEMOCAP1.pkl',
                           'total dataset includes training set, valid set and test set')
tf.app.flags.DEFINE_string(
    'checkpoint', './checkpoint/',   'the checkpoint dir')
tf.app.flags.DEFINE_string('model_name', 'model.ckpt',      'model name')
tf.app.flags.DEFINE_string(
    'pred_name',  './pred0.pkl',        'the test output dir')
tf.app.flags.DEFINE_integer('checkpoint_secs',  60,
                            'checkpoint saving interval in seconds')

# Global Constants
# ================

tf.app.flags.DEFINE_float('dropout_conv',     1,
                          'dropout rate for covvolutional layers')
tf.app.flags.DEFINE_float('dropout_linear',   1,
                          'dropout rate for linear layer')
tf.app.flags.DEFINE_float('dropout_lstm',     1,
                          'dropout rate for lstm')
tf.app.flags.DEFINE_float('dropout_fully1',   1,
                          'dropout rate for fully connected layer1')
tf.app.flags.DEFINE_float('dropout_fully2',   1,
                          'dropout rate for fully connected layer1')
...
tf.app.flags.DEFINE_integer(
    'decay_steps', 570, 'the lr decay_step for optimizer')
tf.app.flags.DEFINE_float('momentum', 0.99, 'the momentum')
tf.app.flags.DEFINE_integer('num_epochs', 30000, 'maximum epochs')
tf.app.flags.DEFINE_float('relu_clip',        20.0,
                          'ReLU clipping value for non-recurrant layers')
...
tf.app.flags.DEFINE_integer('train_batch_size', 40,
                            'number of elements in a training batch')
tf.app.flags.DEFINE_integer('valid_batch_size',   40,
                            'number of elements in a validation batch')
tf.app.flags.DEFINE_integer('test_batch_size',  40,
                            'number of elements in a test batch')
...
class CRNN(object):
    def __init__(self, mode):
        ...
        self.inputs = tf.placeholder(
            tf.float32, [None, FLAGS.image_height, FLAGS.image_width, FLAGS.image_channel])
        # emotion label
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('cnn'):
            with tf.variable_scope('unit-1'):
                x = self._conv2d(self.inputs, 'cnn-1', filter_size,
                                 FLAGS.image_channel, filters[0], filter_strides)
                x = self._batch_norm('bn1', x)
                x = self._leaky_relu(x, 0.01)
                x = self._max_pool(x, pool1_size, pool1_size)
#                print x.get_shape()
            with tf.variable_scope('unit-2'):
                x = self._conv2d(x, 'cnn-2',  filter_size,
                                 filters[0], filters[1], filter_strides)
...
#            print x.get_shape()
        with tf.variable_scope('lstm'):
            ...
            cell_fw = tf.contrib.rnn.BasicLSTMCell(
                FLAGS.cell_num, forget_bias=1.0)
            if self.mode == 'train':
                cell_fw = tf.contrib.rnn.DropoutWrapper(
                    cell=cell_fw, output_keep_prob=FLAGS.dropout_lstm)

            cell_bw = tf.contrib.rnn.BasicLSTMCell(
                FLAGS.cell_num, forget_bias=1.0)
            if self.mode == 'train':
                cell_bw = tf.contrib.rnn.DropoutWrapper(
                    cell=cell_bw, output_keep_prob=FLAGS.dropout_lstm)
        ...
        with tf.variable_scope('time_pooling'):
            if self.attention is not None:
                outputs, alphas = self._attention(
                    outputs, FLAGS.attention_size, return_alphas=True)
            else:
                outputs = tf.concat(outputs, 2)
                outputs = tf.reshape(
                    outputs, [-1, FLAGS.seq_len, 2*FLAGS.cell_num, 1])
                outputs = self._max_pool(
                    outputs, [FLAGS.seq_len, 1], [FLAGS.seq_len, 1])
                outputs = tf.reshape(outputs, [-1, 2*FLAGS.cell_num])
#            print outputs.get_shape()

        with tf.variable_scope('dense'):
            y = self._linear(outputs, 'dense-matmul',
                             [2*FLAGS.cell_num, FLAGS.hidden1])
            y = self._batch_norm_wrapper('dense-bn', y)
            y = self._leaky_relu(y, 0.01)

        self.logits = self._linear(
            y, 'softmax', [FLAGS.hidden1, FLAGS.hidden2])
